 ...
 * or whole octaves up or down from these primitive generators.
 * 
 * For example: new Pitch('C') makes middle C. new Pitch('C').transpose(1) makes
 * C-sharp. new Pitch('F').transpose(-2) makes E-flat. new
 * Pitch('C').transpose(OCTAVE) makes high C. new Pitch('C').transpose(-OCTAVE)
 * makes low C.
 */
public class Pitch {
     ...
     * 
     * Abstraction function AF(value, accidental, octave): AF(scale[C], 0, 0),
     * AF(scale[D], 0, 0), ..., AF(scale[B], 0, 0) map to middle C, D, E, F, G,
     * A, B respectively AF(x, y, 0) maps to y sharps applied to A(x, 0, 0)
     * AF(x, -y, 0) maps to y flats applied to A(x, 0, 0) AF(x, y, n) maps to n
     * octaves above middle AF(x, y, 0) AF(x, y, -n) maps to n octaves below
     * AF(x, y, 0)
     */
    ...
    // NOTE: This visibility of this constructor was changed from private to
    // public by Eric.
    ...
    /**
     * Make a Pitch. Return Pitch named c in the middle octave of the piano
     * keyboard. * For example, new Pitch('C') constructs middle C. 
     ...
     * 
     * @return pitch made by adding semitonesUp sharps to this pitch, if the
     *         argument is positive, or -semitonesUp flats, if the argument is
     *         negative. For example, E transposed by -1 semitone is E flat; E
     *         transposed by 1 semitone is E sharp.
     */
    ...
    /**
     * @return pitch made by transposing this pitch by octavesUp octaves up. For
     *         example, transposing E up by 1 octave produces E' ; transposing E
     *         down by 1 octave produces E, .
     */
    ...
    /**
     * @return pitch made by transposing this pitch up by semitonesUp semitones,
     *         as if on a minor scale starting at this pitch. For example,
     *         transposing E up by 3 semitones will produce G; transposing F up
     *         by 5 semitones will produce B flat.
     */
    public Pitch transpose(int semitonesUp) {
        ...
        if (!isValid(newValue)) {
            ...
            if (interval == 3 || interval == 5 || interval == 8
                    || interval == 10) {
                newValue++;
                newAccidental--;
            } else {
     ...
     *         that.transpose(n).toMidiNote() == this.toMidiNote().
     */
    ...
    public boolean equals(Object obj) {
        ...
        return this.value == that.value && this.accidental == that.accidental
    ...
    private static final String[] valToString = { "C", null, "D", null, "E",
            "F", null, "G", null, "A", null, "B" };
